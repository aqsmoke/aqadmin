		...
		if ($this->_spreadSheet !== NULL) {
			...
			if ($objZip->open($pFilename, ZIPARCHIVE::OVERWRITE) !== true) {
				if ($objZip->open($pFilename, ZIPARCHIVE::CREATE) !== true) {
					exit("Could not open " . $pFilename . " for writing.");
				}
			}
			...
			if ($objZip->close() === false) {
				exit("Could not close zip file $pFilename.");
			}

			// If a temporary file was used, copy it to the correct file stream
			if ($originalFilename != $pFilename) {
				if (copy($pFilename, $originalFilename) === false) {
					exit("Could not copy temporary zip file $pFilename to $originalFilename.");
				}
				@unlink($pFilename);
			}
		} else {
			exit("PHPExcel object unassigned.");
		}
	}
	...
	public function setUseDiskCaching($pValue = false, $pDirectory = null) {
		...
		if ($pDirectory !== NULL) {
    		if (is_dir($pDirectory)) {
    			...
    			exit("Directory does not exist: $pDirectory");
    		}
		}
		return $this;
	}
