...
{	
	...
	function cp_exception_handler(Exception $e) {  
		throw new cpError($e->getMessage(),$e->getCode(),$e->getFile(),$e->getLine());
	}
	/**
	 * 默认错误处理函数
	 */
	function cp_error_handler($errorCode,$errorMessage,$errorFile,$errorLine) {  
		throw new cpError($errorMessage,$errorCode,$errorFile,$errorLine);
	}
	
	set_exception_handler('cp_exception_handler');//注册默认异常处理函数
	set_error_handler('cp_error_handler',E_ALL ^ E_WARNING ^ E_NOTICE);//注册默认错误处理函数
}	
...
<div style=" font-size:15px;">您可以选择 &nbsp;&nbsp;<a href="'.$_SERVER['PHP_SELF'].'" title="重试">重试</a> &nbsp;&nbsp;<a href="javascript:history.back()" title="返回">返回</a>  或者  &nbsp;&nbsp;<a href="'.AQ_SITE.'" title="回到首页">回到首页</a> </div>
</div>
</div>
</body>
</html>';
